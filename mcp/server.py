...
def remove_watermark_legacy(
    ...
    WHEN TO USE THIS TOOL INSTEAD OF remove_watermark:
      Only when the user has confirmed the image was generated by Gemini
      *before* the Gemini 3.5 release. The two profiles use different alpha
      maps and watermark positions; applying the wrong profile produces a
      faint inverted-diamond residue or, more often, simply skips with low
      confidence.

      The standard remove_watermark tool targets the current (Gemini 3.5+)
      profile by default. If it returns skipped=True with a low confidence
      score, do NOT silently retry with this legacy tool -- ask the user
      first whether the image is a pre-Gemini-3.5 output. Two failure
      modes are indistinguishable from the confidence alone:
        - Pre-Gemini-3.5 image  -> this tool will work
        - Not a Gemini image    -> this tool will also skip
