...
def remove_watermark(
    ...
    PROFILE SELECTION (GWT v0.4.0+):
      Default                    → tries the current Gemini 3.5+ profile;
                                   automatically retries with the legacy
                                   (pre-3.5) profile if the current one
                                   skipped. Handles mixed inputs without
                                   user supervision.
      no_legacy=True             → current profile only, no fallback
      remove_watermark_legacy(.) → legacy profile only, no current attempt
...
def remove_watermark_batch(
    ...
    Detection enabled by default — non-watermarked images are skipped
    safely. Each file is tried against the current Gemini 3.5+ profile
    first and falls back to the legacy (pre-3.5) profile on skip; this
    matches the GWT CLI default and handles mixed directories without
    user supervision. Set no_legacy=True to disable the fallback.

    For directories that are known to contain only pre-Gemini-3.5
    outputs, use remove_watermark_batch_legacy instead -- it pins the
    legacy profile and skips the current-profile attempt.
...
# ---------------------------------------------------------------------------
# Tool: remove_watermark_legacy
# ---------------------------------------------------------------------------

@mcp.tool()
def remove_watermark_legacy(
    input_path: str,
    output_path: str,
    # Detection
    force: bool = False,
    ...
    """
    Remove a Gemini watermark using the LEGACY profile (pre-Gemini-3.5 outputs).

    WHEN TO USE THIS TOOL:
      Mostly for the rare case where the caller is certain the image is a
      pre-Gemini-3.5 output and wants to skip the current-profile attempt
      entirely (saves a few milliseconds and the V2 detection log line).

      In v0.4.0+, remove_watermark itself already retries with the legacy
      profile automatically when the current profile skips, so you usually
      do NOT need this tool. Call remove_watermark first; only fall back
      to this tool if you have side knowledge that the image is older
      Gemini and want to skip the V2 attempt.

      If remove_watermark returns skipped=True even after the automatic
      legacy fallback, both profiles failed -- that is the signal to
      surface to the user: the image is either not a Gemini output, or
      its watermark has been destroyed by heavy resizing / re-encoding.
      Do not silently re-invoke this tool in that case.

    All other parameters behave identically to remove_watermark.
    """
    input_abs = str(Path(input_path).resolve())
    output_abs = str(Path(output_path).resolve())
    ...
    args = ["--legacy", "-i", input_abs, "-o", output_abs]
...
# ---------------------------------------------------------------------------
# Tool: remove_watermark_batch_legacy
# ---------------------------------------------------------------------------

@mcp.tool()
def remove_watermark_batch_legacy(
    input_dir: str,
    output_dir: str,
    threshold: float = 0.25,
    denoise: Optional[str] = None,
    ...
    """
    Batch removal using the LEGACY profile (pre-Gemini-3.5 outputs).

    Same as remove_watermark_batch but pins every file in the directory
    to the legacy profile. Use only when the directory contains pre-
    Gemini-3.5 outputs. For a mixed directory, run remove_watermark_batch
    first; files that do not match the current profile will be skipped
    safely (not modified), and a second pass with this legacy tool will
    pick up the older ones.
    """
    input_abs = str(Path(input_dir).resolve())
    output_abs = str(Path(output_dir).resolve())
