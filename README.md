...
🆕 **Gemini 3.5 watermark profile supported** — paired with [GeminiWatermarkTool v0.3.0](https://github.com/allenk/GeminiWatermarkTool/releases/tag/v0.3.0). Default tools target the current Gemini 3.5+ profile; two new `*_legacy` MCP tools opt into the pre-Gemini-3.5 profile when a user confirms an image's source. See [Gemini 3.5 vs Legacy Profile](#scenario-7-pre-gemini-35-image-legacy-profile) below.
...
| `remove_watermark` | V2 (current) | Single file, full parameter control |
| `remove_watermark_snap` | V2 (current) | Single file, resized/recompressed images |
| `remove_watermark_batch` | V2 (current) | Entire directory |
| `remove_watermark_legacy` | V1 (pre-3.5) | Single file, pinned to legacy profile |
| `remove_watermark_batch_legacy` | V1 (pre-3.5) | Entire directory, pinned to legacy profile |

V2 (current) tools target Gemini 3.5 and later outputs and should be the
default choice. The `*_legacy` tools should only be used after the user has
explicitly confirmed the image is a pre-Gemini-3.5 output — see
[Scenario 7](#scenario-7-pre-gemini-35-image-legacy-profile) for the
recommended flow when current-profile detection skips.
...
GeminiWatermarkTool v0.3.0 ships two watermark profiles. The default
(`remove_watermark` / `remove_watermark_batch`) targets **Gemini 3.5 and
later**. Images generated by Gemini *before* the 3.5 release used a
different watermark position and a different alpha map — those need the
**legacy profile**, exposed as `remove_watermark_legacy` and
`remove_watermark_batch_legacy`.

#### When to use the legacy tools

Only after the user has explicitly confirmed the image was generated by
Gemini before the 3.5 release. A failed detection on the current-profile
tool is **not** a reliable signal on its own: a `skipped` result with low
confidence could mean either pre-Gemini-3.5 *or* not a Gemini image at
all. The agent should ask the user instead of silently retrying.

#### Recommended flow when the default tool skips

```text
1. Try current profile first.
   Agent → remove_watermark(input, output)
2. If skipped=True with low confidence, ask the user:
   "I couldn't detect a current-profile Gemini watermark on this image
    (confidence X%). Was this image generated by Gemini *before* the 3.5
    release? If so, I can retry with the legacy profile."
3. Based on the answer:
   - "Yes, it's older Gemini" → remove_watermark_legacy(input, output)
   - "It's not from Gemini"   → stop, do nothing
   - "I don't know"           → try snap / fallback-region first; only
                                escalate to legacy after that also fails
```
